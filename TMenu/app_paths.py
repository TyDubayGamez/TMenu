...
def data_dir() -> str:
    """Folder holding the app's own shipped/persisted JSON data - settings,
    theme, binds, the CHALLENGES presets - as opposed to base_dir() (the
    exe's own folder) or export_dir() (a per-kind folder for things the
    user explicitly exports/imports via a Save/Open dialog).

    Source layout: <repo>/data/. Frozen: <exe folder>/data/, created on
    demand if a fresh install doesn't have one yet (mirrors export_dir()'s
    own os.makedirs behaviour) - see build.bat's "Copying runtime JSON
    files" step for what ships a fresh data/ folder in the first place.
    """
