...
def build_rgb_field_group(tab, win, state, title, address, default=None, parent_layout=None,
                           group_width=250, session_key=None):
    ...
    `session_key`: pass "fog_color"/"skater_color" (matching
    menu_config.VISUALS_RGB_GROUPS) so SET RGB reports here for SAVE
    CONFIG's change-tracking (see session_changes.py) - leave as None for
    an RGB group that isn't part of the whole-menu config."""
    if parent_layout is None:
        layout = QVBoxLayout(tab)
        layout.setAlignment(Qt.AlignHCenter | Qt.AlignTop)
        layout.setContentsMargins(0, 20, 0, 0)
    ...
    def on_set_clicked():
        ...
        try:
            ...
            if session_key is not None:
                session_changes.mark_rgb(session_key, values, default)
