...
def _flip_fire(label, writes, toggles_obj=None):
    ...
    `toggles_obj`, when given, is the exact ONBOARD_TOGGLES/VISUALS_TOGGLES/
    etc. dict `label` came from - reported to session_changes so a hotkey
    firing one of these counts the same as clicking its own tab's button
    for SAVE CONFIG's change-tracking. Left as None for anything that
    isn't part of the whole-menu config (MISC>DEBUG, ONLINE>Freeskate)."""
    def fire(state):
        ...
        try:
            ...
            if toggles_obj is not None:
                session_changes.mark_toggle(toggles_obj, label, turning_on)
...
def _toggles_to_items(toggles: dict) -> dict:
    ...
    "toggle"-kind items. `toggles` itself is threaded into _flip_fire so a
    hotkey reports back to session_changes the same as its tab button."""
    return {
        slug(name): {"label": name, "fire": _flip_fire(name, cfg["writes"], toggles_obj=toggles)}
...
def _score_set(state, value):
    ...
    session_changes.mark_hud_score_multiplier(float(value), SCORE_MULTIPLIER_DEFAULT)
