...
TAB_NAMES = ["CONNECTION", "ONLINE", "ADJUSTABLES", "TOGGLEABLES", "VISUALS", "MISC", "ANIMATIONS", "EDIT SKATER", "PARK", "SAVE", "BINDS", "SETTINGS"]


DEFAULT_SIZE = (750, 467)

# icon.ico (in build/, see this repo's layout) is bundled into the exe as
# PyInstaller data (see build.bat's --add-data) specifically so it's
# available here at runtime to set as the window icon - --icon alone (also
# in build.bat) only bakes it in as the exe's file icon (Explorer/shortcuts),
# it doesn't make a frameless window show it in the taskbar/alt-tab on its own.
WINDOW_ICON_PATH = resource_path("build", "icon.ico")
...
def build_window(state, settings, binds_runtime, rebuild_tabs):
    ...
    animations_subtabs = animations_tab.build(tabs.tab("ANIMATIONS"), win, state)
    ...
    _refit_on_switch(animations_subtabs, win)
...
def main():
    ...
    # Fresh SAVE CONFIG change-log every launch - see session_changes.py's
    # docstring for why SAVE only ever persists what's actually been SET
    # this session, never a blind re-read of everything.
